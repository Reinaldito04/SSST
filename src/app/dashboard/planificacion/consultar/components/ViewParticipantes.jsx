...
function ViewParticipantes({ tasks, open, close, refetch }) {
  ...
  const addParticipant = async (user) => {
    try {
      const response = await axioInstance.post(`/task-plans/assign-participants`, {
        task_plan_id	: tasks.id,
        user_ids: [user.id],
      });
      Swal.fire({
        icon: "success",
  ...
  const deleteParticipant = async (participantId) => {
    try {
      const response = await axioInstance.post(`/task-plans/revoke-participants`, {
        task_plan_id	: tasks.id,
        user_ids	: [participantId.id],
      });
      Swal.fire({
        icon: "success",
      ...
      refetch();
      close();
      participants.filter((participant) => participant.id !== participantId.id);
      ...
      Swal.fire({
        ...
        title: "Error",
        text:
          error?.response?.data?.message || error?.response?.data?.error || "Error al eliminar el participante",
        background: "#f8f9fa",
      });
    }
  };
      ...
      <div className="modal-content border-0">
        ...
        <div className="modal-footer border-0 bg-light">
          ...
          >
            <i
              className={`bi ${
                showAddSection ? "bi-x-circle" : "bi-plus-circle"
              } me-2`}
            ></i>
