...
function ModalTasksFiles({ task, isOpen, onClose }) {
  ...
  const fetchTaskData = async () => {
    ...
    try {
      const response = await axioInstance.get(`/tasks/${task.id}`);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await axioInstance.post("/tasks/assign-files", formData, {
        headers: {
  ...
  const handleDeleteFile = async (file) => {
    try {
      await axioInstance.post(`/tasks/delete-files`, {
        file_ids: [file.id],
        task_id: task.id,
      });
      Swal.fire({
  ...
  return (
    <Modal
      ...
      style={customStyles}
      contentLabel="Archivos de la Tarea"
      ariaHideApp={false}
    >
      <div className="modal-header d-flex justify-content-between align-items-center mb-4 pb-2 border-bottom">
        <h5 className="modal-title font-weight-bold text-dark">
          ...
          <span className="text-primary">{task?.title || "Sin nombre"}</span>
        </h5>
        <button
          onClick={closeModal}
          ...
          <table className="table table-hover mb-0">
            ...
            <tbody>
              {currentTask?.files?.length > 0 ? (
                currentTask.files.map((archivo, index) => (
...
export default ModalTasksFiles;