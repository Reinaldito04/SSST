...
const TaskStatistics = () => {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        ...
        const response = await axioInstance.get(`/tasks?period_filters[0][column]=created_at&period_filters[0][start]=${filters.startDate}&period_filters[0][end]=${filters.endDate}`);
  ...
  return (
    <div className="container mt-4">
      <h1 className="text-center mb-4">Estadísticas de Tareas</h1>
            ...
            <div className="card-body">
              <ul className="list-unstyled">
                {Object.entries(plantStats).map(([plant, count]) => (
                  <li key={plant}>{plant}: <strong>{count}</strong></li>
                ))}
              </ul>
            </div>
          </div>
